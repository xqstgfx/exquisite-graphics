...
function saveSVG(data: string, suiteName: string, name: string) {
  // TODO make dirs correctly, add in rects vs svg
  // mkdir test/svg
...
async function render(
  ...
  console.log(
    'pixel header',
    generatePixelHeader(numCols, numRows, numColors, 1, 0, alpha ? 1 : 0)
  );
...
describe('Renderer', () => {
  ...
  describe(`Header Decode`, function () {
    ...
    // TODO test the header encoding is correct across a bunch of different cases
    // version is correct
    it('Should test Header version is decoded properly', async function () {
    ...
    it('Should test Header dimensions are decoded properly', async function () {
      let header = await renderer.decodeHeader('0x0140400010000003');
    ...
    it('Should test Header colors are decoded properly', async function () {
    ...
    it('Should determine the get the start of the palette and pixel data', async function () {
      let header = await renderer.decodeHeader('0x0140400010000003');
    ...
    it('Should test Header background color index is decoded properly', async function () {
      let header = await renderer.decodeHeader('0x0140400010000003');
    ...
    // alpha is correct
    it('Should test Header alpha is decoded properly', async function () {
      let header = await renderer.decodeHeader('0x0140400010000003');
    ...
    it('Should test Header scale factor is decoded properly', async function () {
      let header = await renderer.decodeHeader('0x0140400010000003');
    ...
    it('Should test Header Background Enabled is decoded properly', async function () {
      let header = await renderer.decodeHeader('0x0140400010000003');
  ...
  describe(`Header Validate`, function () {
    it('Should not revert when number of pixels is within max (4096)', async function () {
      let result = await renderer.validHeader('0x0140400010000003');
      result = await renderer.validHeader('0x0120400010000003');
    });

    it('Should revert when number of pixels exceeds max (4096)', async function () {
      await expect(
        renderer.validHeader('0x0100000010000003')
      ).to.be.revertedWith('ExceededMaxPixels');

      await expect(
        renderer.validHeader('0x0150400010000003')
      ).to.be.revertedWith('ExceededMaxPixels');
    });

    it('Should not revert when number of colors is within max (256)', async function () {
      let result = await renderer.validHeader('0x0140400100000003');
      result = await renderer.validHeader('0x0120400010000003');
    });

    it('Should revert when number of colors exceeds max (256)', async function () {
      await expect(
        renderer.validHeader('0x0140400110000003')
      ).to.be.revertedWith('ExceededMaxColors');

      await expect(
        renderer.validHeader('0x01404001f0000003')
      ).to.be.revertedWith('ExceededMaxColors');
    });

    it('Should not revert when the background color index is in of range or is disabled', async function () {
      let result = await renderer.validHeader('0x0140400010000003');
      result = await renderer.validHeader('0x0140400010010003');
      result = await renderer.validHeader('0x0140400000000000');
    });

    it('Should revert when the background color index is out of range and is enabled', async function () {
      await expect(
        renderer.validHeader('0x0140400000000001')
      ).to.be.revertedWith('BackgroundColorIndexOutOfRange');

      await expect(
        renderer.validHeader('0x0140400010200001')
      ).to.be.revertedWith('BackgroundColorIndexOutOfRange');
    });
  });

  describe.only('Pixels Validate', function () {
    it('Should revert when not enough data is sent', async () => {
      await expect(renderer.valid('0x014040000000000000')).to.be.revertedWith(
        'NotEnoughData'
      );

      await expect(renderer.valid('0x0101010010000000')).to.be.revertedWith(
        'NotEnoughData'
      );

      // 1 color but no palette given
      await expect(
        renderer.valid('0x01050700010000001000000000')
      ).to.be.revertedWith('NotEnoughData');

      // 1 color but missing data in palette
      await expect(
        renderer.valid('0x0105070001000002ff00331000000000')
      ).to.be.revertedWith('NotEnoughData');
    });
  });

  describe.only(`Palette`, function () {
  ...
  describe(`Colors`, function () {
    it(`Should be able to render in 0 Colors`, async function () {
      ...
      // TODO test with alpha = 1, should get the same result
      const header = generatePixelHeader(WIDTH, HEIGHT, NUM_COLORS, 1, 0, 0);
      const pixelData = 'a'.repeat((WIDTH * HEIGHT) / 4);
      const data = `0x${header}${pixelData}`;
    ...
    it(`Should be able to render okpc`, async function () {
      ...
      const image = await renderer.draw(data);
      saveSVG(image, 'COLOR_SWEEP', `OKPC`);
    });
    it.only(`Should be able to render in 1 Color without Background`, async function () {
      // 16x16 test pattern, first pixel on second pixel off.
    ...
    it(`Should be able to render RGB`, async function () {
      // need to validate header and palette data that goes in
    });
  ...
  describe(`Sizes`, function () {
    ...
    it(`Unsafe be able to render in Rectangular sizes up to >4096 pixels total`, async function () {
      const WIDTH = 65;
      const HEIGHT = 65;
      const NUM_COLORS = 64;
  ...
  describe(`SAM`, function () {
    it(`Should render SAM`, async function () {
      ...
      console.log(
        'decoded bytes',
        Buffer.from(SAM.replace('0x', ''), 'hex').byteLength
      );
  ...
  for (let v = 256; v <= 256; v += 1) {
    describe(`56x56 - ${v} Colors`, function () {
      it(`Should render 56x56 with ${v} Colors`, async function () {
        ...
        const done = await render(renderer, 'MAX', HEIGHT, WIDTH, NUM_COLORS);
